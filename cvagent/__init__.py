...
<style>
  ...
  .eu-title{font-size:13px;color:#475569;margin-top:4px}
  .eu-kv{display:grid;grid-template-columns:22px 1fr;gap:10px;margin:6px 0}
  .ico{background:rgba(255,255,255,.18);color:#fff;width:22px;height:22px;border-radius:6px;background:#e2e8f0;display:flex;align-items:center;justify-content:center;font-size:12px}
  .eu-sec{margin-top:16px}
  .eu-sec h2{font-size:14px;font-weight:800;margin:0 0 10px;text-transform:uppercase;letter-spacing:.06em}
  .eu-chip{background:rgba(255,255,255,.18);color:#fff;display:inline-block;background:#eef2ff;color:#3730a3;border:1px solid #e0e7ff;border-radius:999px;padding:3px 10px;margin:3px 6px 0 0;font-size:11px}
  .eu-job{margin:12px 0 10px}
  .line2{color:#64748b;font-size:12px;margin-top:2px}
  .desc{margin-top:6px}
  .eu-job ul{margin:6px 0 0 18px}
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if "cv" in body:
            cv = body["cv"]
            # --- compute output file name: <source-base>-<template>.pdf ---
            src = body.get('source_name') or body.get('pptx_name') or body.get('file_name') or 'cv.pdf'
            try:
                import os
